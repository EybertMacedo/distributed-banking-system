...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    loginForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (role === 'admin') {
          ...
          window.location.href = `http://localhost:4003/clients/${clientId}`;
        } else {
    ...
    depositForm.addEventListener('submit', async (event) => {
      ...
      try {
        await axios.post(`http://localhost:4003/accounts/${accountId}/deposit`, { amount });
    ...
    transferForm.addEventListener('submit', async (event) => {
      ...
      try {
        await axios.post('http://localhost:4003/accounts/${fromAccountId}/transfer/${toAccountId}', { amount });
    ...
    withdrawForm.addEventListener('submit', async (event) => {
      ...
      try {
        await axios.post(`http://localhost:4003/accounts/${accountId}/withdraw`, { amount });
